...
def particleTimeCurve(valueType):
    return timeCurve(particleTime(), valueType) #, "Particle time curves are normalized to the particle's lifetime.")


def emitterTimeCurve(valueType):
    return timeCurve(emitterTime(), valueType) #, "A curve in 'real time', matching against the current emitter's lifetime.")
        ...
        "emitters": {
            ...
            "items": {
                ...
                "properties": merge(
                    ...
                    property("sizeX", 0.0, emitterTimeCurve(number("Defines the particle size in world units."))),
                    property("sizeY", 0.0, emitterTimeCurve(number("Defines the particle size in world units. Y is the same dimension as the velocity facing."))),
                    ...
                    property("red", 0.0, emitterTimeCurve(number("Particle initial color multiplier in linear float color. Set at the time of each particle’s spawn."))),
                    property("green", 0.0, emitterTimeCurve(number("Particle initial color multiplier in linear float color. Set at the time of each particle’s spawn."))),
                    property("blue", 0.0, emitterTimeCurve(number("Particle initial color multiplier in linear float color. Set at the time of each particle’s spawn."))),
                    property("alpha", 0.0, emitterTimeCurve(number("Particle initial color multiplier in linear float color. Set at the time of each particle’s spawn."))),
